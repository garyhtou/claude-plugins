# Design Principles: Visual and Motion Language

Reference for the `high-fidelity-web` skill. The "taste and craft" knowledge base:
what separates premium from template, with concrete numbers, easing values, and
rules you can apply directly. Load in Phase 2 to 3 (concept and system) and again
during Polish, for awwwards-grade marketing/landing sites (the Stripe / Linear /
Vercel / Apple / Column tier).

For just the copy-paste motion tokens, see the leaner `motion-language.md`; this
file is the full craft context behind them.
